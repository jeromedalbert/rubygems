...
module Spec
  module Helpers
    ...
    def bundle(cmd, options = {}, &block)
      ...
      env["BUNDLE_BIN_PATH"] = "#{ruby_cmd} #{bundle_bin}" if preserve_ruby_flags
    ...
    def ruby(ruby, options = {})
      env, ruby_cmd = build_ruby_cmd(options)
    ...
    def build_ruby_cmd(options = {})
      ...
      https_main_source = main_source&.start_with?("https://")

      requires = options.delete(:requires) || []
      artifice = options.delete(:artifice) do
        if current_example && current_example.metadata[:realworld]
          "vcr"
        elsif https_main_source
